...
export function MfaVerify() {
  ...
  const onSubmit = async (data: MfaFormValues) => {
    try {
      ...
      error(apiErr.message || 'Verification failed. Please check the code and try again.');
    } finally {
  ...
  const handleRequestRecovery = async () => {
    ...
    try {
      ...
      const apiErr = err as ApiError;
      error(apiErr.message || 'Failed to send recovery code. Please contact HR.');
    } finally {
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          <div style={{
            ...
            height: '3.5rem',
            borderRadius: '1rem',
            background: isRecoveryMode ? 'rgba(245, 158, 11, 0.1)' : 'rgba(16, 185, 129, 0.1)',
            color: isRecoveryMode ? 'var(--color-warning, #f59e0b)' : 'var(--color-success, #10b981)',
            marginBottom: '1rem',
            transition: 'all 0.3s'
          }}>
            <ShieldCheck size={28} />
        ...
        <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          <div>
            <div style={{ position: 'relative' }}>
              <div style={{ position: 'absolute', left: '1rem', top: '50%', transform: 'translateY(-50%)', color: 'var(--color-text-muted, #94a3b8)', pointerEvents: 'none' }}>
                <KeyRound size={18} />
              </div>
              <input
                {...register('code')}
            ...
            {errors.code && (
              <p style={{ color: 'var(--color-danger, #ef4444)', fontSize: '0.8rem', margin: '0.5rem 0 0 0', textAlign: 'center' }}>
                {errors.code.message}
              </p>
            )}
          </div>
        ...
        <div style={{ textAlign: 'center', margin: '2rem 0 0 0', fontSize: '0.9rem' }}>
          {!isRecoveryMode ? (
            <p style={{ color: 'var(--color-text-muted, #94a3b8)', margin: 0 }}>
              ...
              <button 
                ...
                style={{ 
                  ...
                  border: 'none',
                  padding: 0,
                  color: (isRequestingRecovery || recoveryCooldown > 0) ? 'var(--color-text-muted, #94a3b8)' : 'var(--color-primary, #6366f1)', 
                  fontWeight: 500,
                  cursor: (isRequestingRecovery || recoveryCooldown > 0) ? 'not-allowed' : 'pointer',
                  textDecoration: 'none'
                }}
              >
            ...
            <p style={{ color: 'var(--color-text-muted, #94a3b8)', margin: 0 }}>
              ...
              >
                {recoveryCooldown > 0 ? `Resend in ${recoveryCooldown}s` : 'Resend SMS'}
              </button>
            </p>
          )}
        </div>
